...
with gr.Blocks(
    ...
    with gr.Tabs():
        with gr.TabItem("推理"):
            gr.Markdown(value="""
                So-vits-svc 4.1 模型推理 WebUI
                """)
            with gr.Row(variant="panel"):
                with gr.Column():
                    gr.Markdown(value="""
                        <font size=2>可以上传自己的模型进行推理（上传比较慢），也可以直接使用项目的内置模型</font>
                        """)
                    with gr.Tabs():
                        # invisible checkbox that tracks tab status
                        local_model_enabled = gr.Checkbox(value=False, visible=False)
                        with gr.TabItem('上传自己的模型') as local_model_tab_upload:
                            with gr.Row():
                                model_path = gr.File(label="选择模型文件（必选）")
                                config_path = gr.File(label="选择配置文件（必选）")
                        with gr.TabItem('项目内置模型') as local_model_tab_local:
                            gr.Markdown(f'点击“刷新项目模型列表”后选择项目模型文件夹“{local_model_root}”即可加载可立即进行推理使用的主模型')
                            gr.Markdown(f'如需使用主模型对应的扩散模型和其配置文件（diffusion.yaml)、聚类模型（kmeans_10000.pt）和特征检索模型（feature_and_index.pkl）以增加推理效果，需在空间文件“more_model_file”文件夹内自行下载并上传使用')
                            local_model_refresh_btn = gr.Button('刷新项目模型列表')
                            local_model_selection = gr.Dropdown(label='选择模型文件夹', choices=[], interactive=True)
                    with gr.Row():
                        diff_model_path = gr.File(label="选择扩散模型文件（可选）")
                        diff_config_path = gr.File(label="选择扩散模型配置文件（可选）")
                    cluster_model_path = gr.File(label="选择聚类模型或特征检索文件（可选）")
                ...
                with gr.Column():
                    gr.Markdown(value="""
                        <font size=3>左侧文件选择且上传完毕后，点击“加载模型”进行解析：</font>
                        """)
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("文字转音频"):
                    ...
                    with gr.Row():
                        tts_gender = gr.Radio(label = "选择模型文件说话人的性别", choices = ["男","女"], value = "男")
        ...
        with gr.TabItem("小工具"):
            gr.Markdown(value="""
                        <font size=2> So-vits-svc 4.1 小工具</font>
                        """)
            with gr.Tabs():
